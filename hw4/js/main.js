...
function wordsList(str, subStr) {
    let string = str.replace(/[. , ! ? ; : "]/g, ' ')
        .toLowerCase()
        .split(' ')
        .filter((stringFilter) => stringFilter.indexOf(subStr) > -1);
    let set = new Set()
    string.forEach((value) => set.add(value));
    return set;
}
...
function getLocalDate(date, isSeconds, isISO) {
    ...
    if (isISO) {
        res = `${year}-${month}-${days}, ${hours}:${minutes}`;
    } else {
        res = `${days}.${month}.${year}, ${hours}:${minutes}`;
    }
...
Object.defineProperties(Car.prototype, {
    used: {
        ...
        set(value) {
            ...
            return value === 'new' && this.year !== yearNow ? this.year = yearNow : this.year;
        }
    }
})
